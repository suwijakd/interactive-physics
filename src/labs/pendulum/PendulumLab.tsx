...
import { pendulumLab, type PendulumSettings } from "./pendulumContent";
...
export function PendulumLab({ phase, setPhase }: PendulumLabProps) {
  ...
  const displayRuns = useMemo(() => (simulation.runs.length ? simulation.runs : seedRuns()), [simulation.runs]);
  ...
  }, [displayRuns, phase, simulation.history, simulation.sample]);

  return (
    <>
      {phase === "mission" && <MissionPhase setPhase={setPhase} />}
      {phase === "lab" && (
        <LabPhaseView
...
function MissionPhase({ setPhase }: { setPhase: (phase: LabPhase) => void }) {
  return (
      ...
      <div className="lab-layout">
        <Panel className="control-panel" ariaLabel="Pendulum controls">
          <div className="control-header">
            ...
            <button className="reset-button" onClick={simulation.resetDefaults}>
              Reset to Defaults
            </button>
          </div>
          ...
          <div className="button-row">
            <button className="button green small" onClick={simulation.start}>
              Start
            </button>
            <button className="button blue small" onClick={simulation.togglePause}>
              {simulation.paused ? "Resume" : "Pause"}
            </button>
            <button className="button purple small" onClick={simulation.restart}>
              Restart
            </button>
          </div>
        </Panel>
        ...
        <aside className="right-stack">
          <Panel className="graph-card">
            <div className="card-title-row">
              <h2>Angle vs Time</h2>
              <button className="link-button">Expand</button>
            </div>
      ...
      <div className="lab-bottom">
        <Panel className="lab-challenge">
          ...
          <div className="challenge-actions">
            ...
            <button className="button green small" onClick={simulation.recordRun}>
              {simulation.runs.length >= 3 ? "Replace Oldest Run" : `Record Run${simulation.runs.length ? ` ${simulation.runs.length + 1}` : ""}`}
            </button>
            <button className="button purple small" onClick={() => setPhase("results")}>
              See Results
            </button>
          </div>
        </Panel>
...
function ResultsPhase({
  ...
  displayRuns: ReturnType<typeof seedRuns>;
...
function LabHero({ phase }: { phase: "lab" | "results" }) {
        ...
        <div className="hero-side">
          <aside className="glass-panel stepper-card" aria-label="Phase progress">
            ...
            <div className="stepper">
              {(["mission", "lab", "results"] as const).map((item) => (
                <span key={item} className={`step ${phaseIndex(item) < phaseIndex(currentPhase) ? "done" : ""} ${item === currentPhase ? "active" : ""}`}>
